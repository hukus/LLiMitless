---
title: Dihexa 2026 Scan
created: 2026-04-27
updated: 2026-05-06
type: query
tags: [cognition, peptides, mechanism, preclinical, controversy, safety]
sources: [raw/articles/pubmed-23055539-dihexa-procognitive-antidementia.md, raw/articles/pubmed-34827486-dihexa-app-ps1-memory.md, raw/articles/pubmed-34703584-dihexa-sciatic-nerve-repair.md, raw/articles/pubmed-25649658-hgf-cmet-dihexa-alzheimers.md, raw/articles/fda-bulk-drug-substances-peptide-risk-2026.md, raw/articles/reddit-2026-04-29-dihexa-thread.md, raw/articles/reddit-2026-04-29-dihexa-vs-semax-ultimate-peptide-nootropic-showdown.md]
confidence: low
---

# Dihexa 2026 Scan

Dihexa remains best described as a preclinical AngIV-analog / HGF-cMet candidate with rodent procognitive and recovery-adjacent signals plus unusually persistent gray-market nootropic salience. I rechecked the branch on 2026-05-06 and still did not surface a published human cognition trial or a visible ClinicalTrials.gov Dihexa program. That means the translational status is still preclinical-only even though the community keeps treating it like an "upcoming" cognition peptide.

## What Changed Since 2024

- The main update is not a new human dataset but a cleaner separation between `frontier importance` and `evidence ceiling`. Dihexa still has no formal human cognition evidence, but it remains one of the louder speculative peptides in community use because the "rewiring" story persists and the mechanism is unusually specific.
- The foundational 2012 AngIV-analog paper established oral, BBB-permeant procognitive activity in rodent models, the 2021 APP/PS1 mouse paper added a clearer AD-like memory rescue and PI3K/AKT readout, and the 2021 sciatic-nerve paper extended Dihexa into recovery biology. None of those papers crosses into human cognition evidence.^[raw/articles/pubmed-23055539-dihexa-procognitive-antidementia.md]^[raw/articles/pubmed-34827486-dihexa-app-ps1-memory.md]^[raw/articles/pubmed-34703584-dihexa-sciatic-nerve-repair.md]
- The anecdote layer is now easier to interpret: public discussion treats Semax as the safer, better-understood comparator and Dihexa as the higher-theoretical-upside, higher-theoretical-risk option.^[raw/articles/reddit-2026-04-29-dihexa-vs-semax-ultimate-peptide-nootropic-showdown.md]^[raw/articles/reddit-2026-04-29-dihexa-thread.md]

## Top Sources

- `raw/articles/pubmed-23055539-dihexa-procognitive-antidementia.md` - foundational AngIV-analog / dihexa procognitive paper.
- `raw/articles/pubmed-34827486-dihexa-app-ps1-memory.md` - APP/PS1 mouse memory recovery and PI3K/AKT study.
- `raw/articles/pubmed-34703584-dihexa-sciatic-nerve-repair.md` - rat sciatic-nerve repair study with Dihexa in combination arms.
- `raw/articles/pubmed-25649658-hgf-cmet-dihexa-alzheimers.md` - HGF/c-Met framing for the Alzheimer target discussion.
- `raw/articles/fda-bulk-drug-substances-peptide-risk-2026.md` - current FDA read on Dihexa acetate as a low-data peptide with no identified human exposure data.
- `raw/articles/reddit-2026-04-29-dihexa-vs-semax-ultimate-peptide-nootropic-showdown.md` - practical community framing of Dihexa versus Semax.
- `raw/articles/reddit-2026-04-29-dihexa-thread.md` - route and delivery speculation showing how unsettled real-world Dihexa use still is.

## Evidence Tiering

| Tier | Read |
|---|---|
| Human | No published human cognition trial surfaced in the current check. |
| Registry | No visible ClinicalTrials.gov Dihexa program surfaced in the current check. |
| Animal | Rodent procognitive and recovery-adjacent effects are the only real signal. |
| Mechanistic | HGF/c-Met, PI3K/AKT, synaptogenesis, and anti-inflammatory readouts dominate. |
| Anecdote | Persistent but thin; serious enough for frontier tracking, still too small and too confounded for efficacy confidence. |

## Effect Size Snapshot

There is no human effect size to report. The rodent literature is directionally positive, but the practical meaning is still limited by the small number of studies, the concentration of work in a narrow research lineage, and the fact that the outcomes are animal behavioral proxies rather than clinical cognition endpoints.^[raw/articles/pubmed-23055539-dihexa-procognitive-antidementia.md]^[raw/articles/pubmed-34827486-dihexa-app-ps1-memory.md]

## Safety / Adverse Events

No human safety dataset was found. That is the key safety issue: Dihexa is still a research compound with no published human PK, tolerability, or adverse-event series in the cognition space. FDA's current bulk page reinforces that by stating that it has not identified human exposure data on drug products containing Dihexa acetate.^[raw/articles/fda-bulk-drug-substances-peptide-risk-2026.md]

The animal literature uses nontrivial mg/kg dosing and experimental formulations, including oral dosing in mice and hydrogel / intraperitoneal / muscle delivery in rats.^[raw/articles/pubmed-34827486-dihexa-app-ps1-memory.md]^[raw/articles/pubmed-34703584-dihexa-sciatic-nerve-repair.md]

## Responder Phenotypes

- APP/PS1-like Alzheimer mouse models.
- Aged rat and scopolamine-impaired rodent models.
- Peripheral nerve injury / repair models.
- Community framing most often centers long-cycle recovery, brain-fog repair, or post-setback restoration rather than a quick stimulant-like effect.^[raw/articles/reddit-2026-04-29-dihexa-vs-semax-ultimate-peptide-nootropic-showdown.md]

No human responder phenotype is established.

## Protocol Parameters

- Foundational procognitive work: metabolically stabilized AngIV analogs with oral BBB-permeant behavior in rodent models.^[raw/articles/pubmed-23055539-dihexa-procognitive-antidementia.md]
- APP/PS1 mouse study: 1.44 mg/kg and 2.88 mg/kg Dihexa were tested in the mouse protocol.^[raw/articles/pubmed-34827486-dihexa-app-ps1-memory.md]
- Rat sciatic-nerve repair study: 2-4 mg/kg dosing was used across local, systemic, and muscle-delivery arms.^[raw/articles/pubmed-34703584-dihexa-sciatic-nerve-repair.md]
- Community discussion often shifts toward low-frequency cycles and route improvisation, which is itself a signal that there is no validated human protocol.^[raw/articles/reddit-2026-04-29-dihexa-thread.md]

## Anecdote Ledger

| Claim or candidate | Evidence layer | Source and source type | Signal size/strength | Context | Confidence | Caveat |
|---|---|---|---|---|---|---|
| Dihexa is the higher-upside but higher-risk alternative to Semax | anecdote | `raw/articles/reddit-2026-04-29-dihexa-vs-semax-ultimate-peptide-nootropic-showdown.md`; Reddit comparison thread | 1 long post plus comments; moderate | Semax comparison, source quality, cycle length | low | Thread is heavy on theory and protocol lore rather than denominator-backed outcomes |
| Dihexa use is dominated by route optimization talk and subtle-effect uncertainty | anecdote | `raw/articles/reddit-2026-04-29-dihexa-thread.md`; Reddit thread | 1 thread; weak | Oral nonresponse, nasal/DMSO/transdermal speculation | low | More evidence of unsettled use than of consistent benefit |

## Upcoming Status

Dihexa currently looks `upcoming as a frontier idea`, not `upcoming as a formal program`. That is the right way to place it beside [[semax]], [[selank]], [[pinealon]], [[mots-c]], and [[pe-22-28]]. It is important because it keeps surviving as a serious candidate in community ranking and mechanism discussions, not because it has crossed any formal evidence threshold.

## Open Contradictions

- The loudest public claims about Dihexa are potency and nootropic strength, but the actual evidence base is still narrow and preclinical.
- Mechanism framing is not settled. Older AngIV/IRAP language and newer HGF/c-Met framing both circulate, but neither has human cognition validation.
- The recovery-oriented animal studies do not yet justify healthy-adult enhancement claims.
- Persistent community heat raises attention priority, but it may still be mostly expectancy, route experimentation, or sourcing selection rather than a robust human effect.

## Actionable Wiki Updates

- Keep [[dihexa]] marked as low-confidence and preclinical.
- Keep linking [[dihexa]] from [[peptide-neuroprotection]], [[nootropic-peptide-anecdote-frontier-2026-scan]], and [[neurocognitive-interventions-under-study]] as a high-salience speculative leaf, not a validated enhancer.
- Use this scan as the current answer whenever Dihexa is mentioned in the cognition backlog or in "upcoming peptide" sweeps.
