---
title: Paraxanthine
created: 2026-05-03
updated: 2026-05-03
type: entity
tags: [domain-medicine, cognition, performance, clinical-trial, safety, dosage, anecdote, mechanism, controversy]
sources: [raw/articles/pmc-8708375-paraxanthine-dose-response-cognition-rct.md, raw/articles/pmc-12777857-paraxanthine-postprandial-cognition-rct.md, raw/articles/pubmed-38725238-paraxanthine-exercise-cognition-caffeine-rct.md, raw/articles/frontiers-2023-paraxanthine-safety-caffeine-comparison.md, raw/articles/nutraingredients-2021-enfinity-paraxanthine-self-affirmed-gras.md, raw/articles/enfinity-2026-paraxanthine-product-visibility.md, raw/articles/rarebird-2026-paraxanthine-coffee-product-identity.md, raw/articles/reddit-2023-paraxanthine-experience-thread.md, raw/articles/reddit-2026-paraxanthine-supplements-experience-thread.md]
confidence: low
contested: true
contradictions: [small positive direct cognition trials vs no large independent replication, caffeine-alternative marketing vs stimulant physiology and dose/sleep constraints, product-specific ENFINITY and Rarebird exposures vs generic supplement labels, smoother-energy anecdotes vs no-effect and dose-escalation reports]
---

# Paraxanthine

Paraxanthine (1,7-dimethylxanthine; PX/PXN) is the main human metabolite of caffeine and is now sold directly as a caffeine-alternative stimulant / nootropic ingredient, especially as ENFINITY or Px coffee. This page covers direct paraxanthine cognition and performance claims, not [[caffeine]], theobromine, theophylline, methylliberine / Dynamine, theacrine, decaf coffee alone, or multi-stimulant energy drinks unless paraxanthine is the active unit.

Medicinal cognition signal: `low-moderate/product-specific and short-term`. The best formal evidence is direct but small. A 12-person double-blind crossover dose-response study found selected acute cognition, memory, reasoning, reaction-time, and sustained-attention signals at 100-200 mg ENFINITY paraxanthine, with no clinically significant side-effect or chemistry-panel signal over 7 days. A 24-person postprandial crossover study found paraxanthine effects on subjective energy/focus/productivity/satiety and cognition tests after a mixed meal, but it was still acute, product-linked, and industry-linked.^[raw/articles/pmc-8708375-paraxanthine-dose-response-cognition-rct.md]^[raw/articles/pmc-12777857-paraxanthine-postprandial-cognition-rct.md]

The exercise-fatigue signal is relevant but state-bound. A 12-runner crossover trial compared placebo, caffeine, paraxanthine, and paraxanthine plus caffeine around a 10-km run and reported greater post-exercise cognitive-function improvement for paraxanthine than caffeine. That supports paraxanthine as a fatigue / post-load comparator, not as proof of durable baseline enhancement.^[raw/articles/pubmed-38725238-paraxanthine-exercise-cognition-caffeine-rct.md]

Safety signal: `low-moderate, mostly preclinical and short-term human`. A 2023 toxicology comparison reported no mutagenicity/genotoxicity signals in standard assays, an acute rat LD50 of 829.20 mg/kg, no treatment-related 14-day rat findings at 50-150 mg/kg, and a 90-day rat NOAEL of 185 mg/kg versus 150 mg/kg for caffeine. Human cognition trials were short and small. This does not remove ordinary stimulant cautions around sleep, blood pressure, heart rate, anxiety, pregnancy/breastfeeding, medications, or high-dose daily use.^[raw/articles/frontiers-2023-paraxanthine-safety-caffeine-comparison.md]

Product identity is central. ENFINITY is the branded paraxanthine ingredient used in several trials and current energy products. Rarebird Px coffee is a separate consumer format: decaffeinated coffee infused with synthetic paraxanthine, listed as 60 mg per serving with a 300 mg/day ceiling on the product page. A 2021 trade source reports self-affirmed GRAS status for ENFINITY up to 300 mg in selected food/beverage uses, but this is not an FDA efficacy approval or a guarantee that all supplement labels deliver the stated dose.^[raw/articles/nutraingredients-2021-enfinity-paraxanthine-self-affirmed-gras.md]^[raw/articles/enfinity-2026-paraxanthine-product-visibility.md]^[raw/articles/rarebird-2026-paraxanthine-coffee-product-identity.md]

Anecdote signal: `weak-moderate for smoother caffeine-like energy, mixed for dose and no-effect`. Community reports include caffeine-like wakefulness without the same jittery or strung-out feel during long work shifts, no noticeable effect at 200 mg, needing about 300 mg to approximate coffee, gentler crash, and label/serving-size confusion. Anecdote is useful for dose and product-gate design but cannot upgrade the formal signal beyond small-trial low-moderate.^[raw/articles/reddit-2023-paraxanthine-experience-thread.md]^[raw/articles/reddit-2026-paraxanthine-supplements-experience-thread.md]

## Signal Snapshot

| Layer | Signal strength | Notes |
|---|---|---|
| Dose-response cognition trial | `low-moderate` | 12 healthy adults; acute and 7-day ENFINITY exposure; selected cognitive endpoints positive |
| Postprandial cognition trial | `low-moderate` | 24 healthy adults; 200/300 mg before meal; subjective energy/focus and small cognition battery |
| Exercise-fatigue comparison | `low/state-bound` | 12 trained runners; post-10-km cognition comparison against caffeine |
| Safety/toxicology | `low-moderate` | Standard genotox and rat toxicity package; human long-term safety still sparse |
| Product identity | `moderate for branded products, weak for generic labels` | ENFINITY and Rarebird are identifiable; consumer supplement labels and stacks need dose verification |
| Anecdote | `weak-moderate, mixed` | Smoother energy reports coexist with no-effect and dose-escalation reports |

## Anecdote Ledger

| Claim | Anecdote type | Source | Reported effect | Direction | Confidence | Why it does or does not matter |
|---|---|---|---|---|---|---|
| 300 mg paraxanthine felt like caffeine-like wakefulness without the same jittery edge | caffeine-alternative self-report | raw/articles/reddit-2023-paraxanthine-experience-thread.md; Reddit /r/Nootropics | long-shift wakefulness, less jittery/strung-out than caffeine | positive | low | Sleep-deprivation, energy-drink formulation, and expectation confound |
| 200 mg did little, while about 300 mg felt closer to caffeine with gentler crash | dose-finding self-report | raw/articles/reddit-2026-paraxanthine-supplements-experience-thread.md; Reddit /r/Supplements | no effect at 200 mg for one user; another needed about 300 mg for coffee-like effect | mixed | low | Recent and useful for dose/product gate, but no objective cognition and possible vendor participation |

## Identity Gates

- Separate paraxanthine from [[caffeine]], decaf coffee, theobromine, theophylline, theacrine, methylliberine / Dynamine, and multi-stimulant blends.
- Track dose and format: 50 mg, 100 mg, 200 mg, 300 mg, 60 mg/cup Px coffee, energy drinks, capsules, powders, and caffeine+paraxanthine combinations are not interchangeable.
- Check product identity: ENFINITY, MuscleTech paraxanthine, Rarebird Px coffee, UPDATE, generic capsules, and multi-ingredient drinks have different transportability.
- Treat paraxanthine as a stimulant: watch sleep timing, blood pressure, heart rate, anxiety/panic sensitivity, pregnancy/breastfeeding, age under 18, stimulant medication, arrhythmia/cardiovascular disease, CYP1A2-active substances, and total caffeine/paraxanthine exposure.
- Do not equate "less jittery than caffeine" with non-stimulant safety.

## Practical Ranking

Paraxanthine is the strongest candidate in the stimulant-adjacent watchlist for standalone tracking because it has direct human cognition trials rather than only lore. It still stays below [[caffeine]], [[modafinil]], [[methylphenidate]], and established state-restoration comparators because the evidence is small, short, product-linked, and not yet independently replicated at scale. The current route is `caffeine-adjacent comparator / short-term focus-energy candidate`, with safety and product identity carried beside the signal.

Related pages: [[stimulant-adjacent-dopamine-precursor-watchlist]], [[cognitive-improvement-molecule-signal-atlas]], [[pharmacology-comparator-map]], [[regulatory-and-sports-visibility-map]], [[caffeine]], and [[biohacker-anecdote-methodology]].

## Source Notes

- raw/articles/pmc-8708375-paraxanthine-dose-response-cognition-rct.md
- raw/articles/pmc-12777857-paraxanthine-postprandial-cognition-rct.md
- raw/articles/pubmed-38725238-paraxanthine-exercise-cognition-caffeine-rct.md
- raw/articles/frontiers-2023-paraxanthine-safety-caffeine-comparison.md
- raw/articles/nutraingredients-2021-enfinity-paraxanthine-self-affirmed-gras.md
- raw/articles/enfinity-2026-paraxanthine-product-visibility.md
- raw/articles/rarebird-2026-paraxanthine-coffee-product-identity.md
- raw/articles/reddit-2023-paraxanthine-experience-thread.md
- raw/articles/reddit-2026-paraxanthine-supplements-experience-thread.md
